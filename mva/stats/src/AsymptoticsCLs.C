...
class AsymptoticsCLs
{
    ...
    double findCrossing(double sigma_obs, double sigma, double muhat)
    {
        ...
        while (fabs(mu_guess-mu_pre) > direction*mu_guess*precision)
        {
            ...
            if (verbose)
                cout << "mu_guess = " << mu_guess
                     << ", mu_pre = " << mu_pre
                     << ", qmu = " << qmu
                     << ", qmu95 = " << qmu95
                     << ", sigma_obs_extrap = " << sigma_obs_extrap
                     << ", sigma = " << sigma
                     << ", direction*mu*prec = " << direction * mu_guess * precision
                     << endl;
        }
