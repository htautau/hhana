...
namespace LimitCrossCheck{  
  ...
  void PlotFitCrossChecks(const char* infile          = "WorkspaceForTest1.root",
    ...
    // -----------------------------------------------------------------------------------
    // 2 - Control plots for systematic morphing (ie, -1/0/+1 sigma --> continuous NP)
    // -----------------------------------------------------------------------------------    
  ...
  void PlotHistosBeforeFit(double nSigmaToVary, double mu){
	...
	for (int ib=0 ; ib<hdata->GetNbinsX() ; ib++) hdata->SetBinError(ib, sqrt(hdata->GetBinContent(ib)));
  ...
  TCanvas* DrawShift(TString channel, TString var, TString comp, double mu, TH1* d, TH1* n, TH1* p1s, TH1* m1s) {
    ...
    TCanvas* canvas = new TCanvas( cname );
    ...
    if(plotRelative) {
      ...
      p1s->SetMaximum( 1.2*max );
      p1s->SetMinimum( min - 0.2*fabs(min) );
      canvas->cd();
      p1s->Draw("hist");
      m1s->Draw("hist same");
  ...
  void PlotHistosAfterFitEachSubChannel(bool IsConditionnal, double mu){
	...
	h1Dpull_axis->GetXaxis()->SetBinLabel(ib,var->GetName());
      }
    ...
    for (int isys=0 ; isys<Nsyst ; isys++){
      ...
      TCanvas* can = new TCanvas( "can_"+ hname, "can_"+hname, 600, 500);
